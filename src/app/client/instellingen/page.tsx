...
export default function ClientInstellingenPage() {
  const { user } = useAuth();
  const [subTab, setSubTab] = useState("profiel");
  const [form, setForm] = useState({ name: "", tel: "", adres: "", postcode: "", plaats: "" });
  ...
  useEffect(() => {
    if (!user) return;
    const od = (user.onboarding_data as Record<string, string | boolean>) || {};
    setForm({
      name: user.name || "",
      tel: (od.tel as string) || "",
      adres: (od.adres as string) || "",
      postcode: (od.postcode as string) || "",
      plaats: (od.plaats as string) || "",
    });
    setPrefs({ emailNotif: od.emailNotif !== false, compact: !!od.compact });
  }, [user]);

  const upd = (k: string, v: string) => { setForm((f) => ({ ...f, [k]: v })); setSaved(false); };
  const updPref = (k: string, v: boolean) => { setPrefs((p) => ({ ...p, [k]: v })); setSaved(false); };

  const handleSave = async () => {
    if (!user) return;
    setSaving(true);
    const supabase = createClient();
    const od = { ...(user.onboarding_data as Record<string, unknown> || {}), tel: form.tel, adres: form.adres, postcode: form.postcode, plaats: form.plaats, emailNotif: prefs.emailNotif, compact: prefs.compact };
    await supabase.from("profiles").update({ name: form.name, onboarding_data: od }).eq("id", user.id);
    setSaving(false);
    setSaved(true);
  };
  ...
  const fieldRow = (label: string, key: keyof typeof form, type = "text", placeholder = "") => (
    <div style={{ marginBottom: 16 }}>
      <label style={lbl}>{label}</label>
      <input type={type} value={form[key]} onChange={(e) => upd(key, e.target.value)} placeholder={placeholder} style={inp} />
    </div>
  );
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          ...
          {!saved && (
            <button onClick={handleSave} disabled={saving}
              style={{ padding: "10px 22px", background: "rgba(198,165,107,.15)", border: `1px solid ${C.gold}`, color: C.gold, fontSize: "0.64rem", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", borderRadius: 8 }}>
              {saving ? "Opslaan…" : "💾 Wijzigingen opslaan"}
            </button>
          )}
          {saved && <span style={{ fontSize: "0.62rem", color: C.green }}>✓ Alles opgeslagen</span>}
        </div>

        {/* Sub-tabs */}
        <div style={{ display: "flex", borderBottom: `1px solid ${C.bdr}`, marginBottom: 28, overflowX: "auto" }}>
          {SUB_TABS.map((t) => (
        ...
        <div style={{ maxWidth: 560 }}>
          ...
          {subTab === "profiel" && (
            <div style={{ animation: "slideUp .2s ease" }}>
              <div style={{ background: C.deep, border: `1px solid ${C.bdr}`, borderRadius: 12, padding: "22px 24px", marginBottom: 16 }}>
                <div style={{ fontSize: "0.5rem", letterSpacing: 3, color: C.gold, textTransform: "uppercase", marginBottom: 18 }}>Persoonlijke gegevens</div>
                {fieldRow("Volledige naam", "name", "text", "Voor- en achternaam")}
                <div style={{ marginBottom: 16 }}>
                ...
                {fieldRow("Telefoonnummer", "tel", "tel", "06 00 00 00 00")}
                {fieldRow("Straat + huisnummer", "adres", "text", "Hoofdstraat 1")}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: 12 }}>
                  <div><label style={lbl}>Postcode</label><input value={form.postcode} onChange={(e) => upd("postcode", e.target.value)} placeholder="1234 AB" style={inp} /></div>
                  <div><label style={lbl}>Stad</label><input value={form.plaats} onChange={(e) => upd("plaats", e.target.value)} placeholder="Rotterdam" style={inp} /></div>
                </div>
              </div>
            </div>
          )}
              ...
              <div style={{ background: C.deep, border: `1px solid ${C.bdr}`, borderRadius: 12, padding: "22px 24px", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                  <div>
                    ...
                    <div style={{ fontSize: "0.6rem", color: C.dim }}>Ontvang updates over uw opdrachten per e-mail</div>
                  </div>
                  <ToggleSwitch value={prefs.emailNotif !== false} onChange={(v) => updPref("emailNotif", v)} />
                </div>
              </div>
              <div style={{ background: C.deep, border: `1px solid ${C.bdr}`, borderRadius: 12, padding: "22px 24px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                  ...
                  <ToggleSwitch value={!!prefs.compact} onChange={(v) => updPref("compact", v)} />
                </div>
              </div>
            </div>
          )}
